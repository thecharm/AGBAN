...
class Data:
    def __init__(self):
        ...
        self.imgId_to_vfeature = {}
        self.imgId_to_vlabel = {}
        self.max_obj = 3
        ...
        self.test_dir = None
        self.raw_dir = None
        self.visual_feature_dir = './data/mul_tweets/object_features_15.txt'
        ...
        self.use_crf = True
        self.nbest = None
        self.v_loss_lambda = 1
        self.t_loss_lambda = 1
        self.ner_loss_lambda = 1
        self.glimpse = 4
    ...
    def show_data_summary(self):
        ...
        print("visual feature file directory: %s"%(self.visual_feature_dir))
        ...
        print("     Visual  loss  lambda: %s" % (self.v_loss_lambda))
        print("     Textual loss  lambda: %s" % (self.t_loss_lambda))
        print("     NER     loss  lambda: %s" % (self.ner_loss_lambda))
        print("     bilinear ATT glimpse: %s" % (self.glimpse))
        if self.use_char:
        ...
        print("     Frozen   clf    steps: %s"%(self.frozen_step))

        print(" "+"++"*20)
        print(" Hyperparameters:")
        print("     Hyper   object number: %s"%(self.max_obj))
        print("     Frozen   clf    steps: %s"%(self.frozen_step))
        print("     bilinear ATT  glimpse: %s"%(self.glimpse))
    ...
    def generate_instance(self, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance(self.train_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, imgId_to_vfeature=self.imgId_to_vfeature, imgId_to_objs=self.imgId_to_vlabel)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_instance(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, imgId_to_vfeature=self.imgId_to_vfeature, imgId_to_objs=self.imgId_to_vlabel)
        elif name == "test":
            self.test_texts, self.test_Ids = read_instance(self.test_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, imgId_to_vfeature=self.imgId_to_vfeature, imgId_to_objs=self.imgId_to_vlabel)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = read_instance(self.raw_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, imgId_to_vfeature=self.imgId_to_vfeature, imgId_to_objs=self.imgId_to_vlabel)
        else:
    ...
    def read_config(self,config_file):
        ...
        if the_item in config:
            self.model_name = config[the_item]
        
        the_item = 'log_root'
        if the_item in config:
            self.log_root = config[the_item]
        the_item = 'train_dir'
        if the_item in config:
        ...
        the_item = 'v_loss_lambda'
        if the_item in config:
            self.v_loss_lambda = float(config[the_item])
        the_item = 't_loss_lambda'
        if the_item in config:
            self.t_loss_lambda = float(config[the_item])
        the_item = 'ner_loss_lambda'
        if the_item in config:
            self.ner_loss_lambda = float(config[the_item])
        the_item = 'glimpse'
        if the_item in config:
            self.glimpse = int(config[the_item])
        the_item = 'max_obj'
        if the_item in config:
            self.max_obj = int(config[the_item])
        ...
        the_item = 'frozen_step'
        if the_item in config:
            self.frozen_step = float(config[the_item])
    ...
    def build_visual_features(self):
        print('Build ImageID to visual features dict ... \n')
        with open(self.visual_feature_dir, 'r', encoding='utf-8') as f:
            image_id = -1
            line_cnt = 0
            for line in f.readlines():
                ...
                if line_cnt == 1:
                    ...
                    visual_feat = [float(i) for i in line.split()]
                    self.imgId_to_vfeature[str(image_id)] = visual_feat
                    self.imgId_to_vlabel[str(image_id)] = img_label
                elif line_cnt == 4:
                    image_id = -1
                    line_cnt = 0

        print('total image number: {} \n'.format(len(self.imgId_to_vfeature)))
