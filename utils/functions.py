...
def read_instance(input_file, word_alphabet, char_alphabet, feature_alphabets, label_alphabet, number_normalized,
                  ...
                  char_padding_symbol='</pad>', imgId_to_vfeature=None, imgId_to_objs=None):
    ...
    char_Ids = []
    label_Ids = []
    visual_features = []  # the visual feature for sent, [f_num] * 2048
            ...
            if len(line) > 2:
                if len(line.strip().split('\t')) >= 2:  # 判断是否是IMAGE ID
                    ...
                    if line.strip().split(':')[0] == 'IMGID':
                        if imgId_to_vfeature is not None:
                            imgId = line.strip().split(':')[1]
                            visual_features = imgId_to_vfeature[imgId]
                            visual_objs = imgId_to_objs[imgId]
            else:
                if (len(words) > 0) and ((max_sent_length < 0) or (len(words) < max_sent_length)):
                    instence_texts.append([words, features, chars, labels, visual_objs])
                    instence_Ids.append([word_Ids, feature_Ids, char_Ids, label_Ids, visual_features])
                words = []
                features = []
                chars = []
                labels = []
